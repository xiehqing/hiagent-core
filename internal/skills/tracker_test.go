...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (hiagent-config).
	activeSkills := []*Skill{
		{Name: "hiagent-config", Description: "HiAgent config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("hiagent-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via hiagent://...).
	tracker.MarkLoaded("hiagent-config")
	require.True(t, tracker.IsLoaded("hiagent-config"))
