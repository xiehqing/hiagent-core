...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hiagent.log")
...
func TestNewHiagentLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewHiagentLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, HiagentLogsToolName, tool.Info().Name)
}

func TestHiagentLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 3})
...
func TestHiagentLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 0})
...
func TestHiagentLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestHiagentLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runHiagentLogs("/nonexistent/path/hiagent.log", HiagentLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestHiagentLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hiagent.log")
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestHiagentLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hiagent.log")
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 10})
...
func TestHiagentLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 1})
...
func TestHiagentLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHiagentLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHiagentLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestHiagentLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hiagent.log")
...
func TestHiagentLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "hiagent.log")
...
func TestHiagentLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHiagentLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestHiagentLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHiagentLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHiagentLogs(logFile, HiagentLogsParams{Lines: 5})
...
func TestHiagentLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
