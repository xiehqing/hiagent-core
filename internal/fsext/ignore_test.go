...
func TestHiAgentIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .hiagentignore file that ignores .log files
	require.NoError(t, os.WriteFile(".hiagentignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .hiagentignore file
	hiagentignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".hiagentignore"), []byte(hiagentignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .hiagentignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .hiagentignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .hiagentignore in subdir that ignores normal_nested
	subHiAgentignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".hiagentignore"), []byte(subHiAgentignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .hiagentignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .hiagentignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .hiagentignore")
